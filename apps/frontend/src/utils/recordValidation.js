 ...
 *   (tentative_date_to_fill_up is not required when status_of_vacancy is one of
 *   NA_TENTATIVE_DATE_STATUSES or OPTIONAL_TENTATIVE_DATE_STATUSES)
 */
export function checkRecordRequiredFields(record = {}, stagedEditsForRow = {}) {
  ...
  if (posStatus === 'FILLED') {
    ...
    // FILLED is a deliberate dropdown choice away from the UNFILLED default,
    // so any row in this branch already has user input worth flagging.
    isUntouched = false;
  } else {
    ...
    const tentativeNotRequired = NA_TENTATIVE_DATE_STATUSES.includes(status) || OPTIONAL_TENTATIVE_DATE_STATUSES.includes(status);
    if (!tentative && !tentativeNotRequired) missingFields.push('Tentative Date to Fill-Up');

    // A brand-new UNFILLED row with none of its vacancy fields entered yet
    // isn't "Incomplete" in the actionable sense — it just hasn't been started.
    isUntouched = !dateVacancy && !reason && !status && !tentative;
  }
