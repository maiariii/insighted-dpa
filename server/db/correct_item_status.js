...
/**
 * Robust CSV line parser handling quoted values with embedded commas
 */
...
async function correctItemStatuses() {
  ...
  // Confirm exact column names for item_number and status field
  let itemNumIdx = rawHeaders.indexOf("item_number");
  let itemStatusIdx = rawHeaders.indexOf("item_status");

  // Fallback check for variant names if necessary
  if (itemNumIdx === -1) {
  ...
  // Build CSV lookup Map<item_number, csv_status>
  ...
  try {
    ...
    for (const dbRow of dbRows) {
      ...
      if (csvStatus !== undefined) {
        ...
        // Check if DB status differs from CSV status
    ...
    if (isCommit) {
      ...
      console.log(`   To execute and apply changes to DB, run: node server/db/correct_item_status.js --commit`);
    }
